...
#akchr1
filterlist1 <- c("1:105400000:120600000")
##Select only protein-coding from Akey
akchr1=getBM(attributes = c("hgnc_symbol", "chromosome_name", "start_position", "end_position","gene_biotype"),
              filters = c("chromosomal_region","biotype"),
              values = list(chromosomal_region=filterlist1,biotype="protein_coding"), mart = ensembl)
akchr1 <- akchr1[!duplicated(akchr1$hgnc_symbol),]

length(akchr1$hgnc_symbol[!(is.na(akchr1$hgnc_symbol) | akchr1$hgnc_symbol=="")])
akchr1 <-  akchr1[!(is.na(akchr1$hgnc_symbol) | akchr1$hgnc_symbol==""), ] #Cleaning
#akchr3
filterlist3 <- c("3:74100000:89300000")
##Select only protein-coding from Akey
akchr3=getBM(attributes = c("hgnc_symbol", "chromosome_name", "start_position", "end_position","gene_biotype"),
              filters = c("chromosomal_region","biotype"),
              values = list(chromosomal_region=filterlist3,biotype="protein_coding"), mart = ensembl)
akchr3 <- akchr3[!duplicated(akchr3$hgnc_symbol),]
length(akchr3$hgnc_symbol[!(is.na(akchr3$hgnc_symbol) | akchr3$hgnc_symbol=="")])
akchr3 <-  akchr3[!(is.na(akchr3$hgnc_symbol) | akchr3$hgnc_symbol==""), ] #Cleaning
#akchr7
filterlist7 <- c("7:106200000:123200000")
##Select only protein-coding from Akey
akchr7=getBM(attributes = c("hgnc_symbol", "chromosome_name", "start_position", "end_position","gene_biotype"),
              filters = c("chromosomal_region","biotype"),
              values = list(chromosomal_region=filterlist7,biotype="protein_coding"), mart = ensembl)
akchr7 <- akchr7[!duplicated(akchr7$hgnc_symbol),]
length(akchr7$hgnc_symbol[!(is.na(akchr7$hgnc_symbol) | akchr7$hgnc_symbol=="")])
akchr7 <-  akchr7[!(is.na(akchr7$hgnc_symbol) | akchr7$hgnc_symbol==""), ] #Cleaning

#akchr8
filterlist8 <- c("8:49400000:66500000")
##Select only protein-coding from Akey
akchr8=getBM(attributes = c("hgnc_symbol", "chromosome_name", "start_position", "end_position","gene_biotype"),
              filters = c("chromosomal_region","biotype"),
              values = list(chromosomal_region=filterlist8,biotype="protein_coding"), mart = ensembl)
akchr8 <- akchr8[!duplicated(akchr8$hgnc_symbol),]
#255
length(akchr8$hgnc_symbol[!(is.na(akchr8$hgnc_symbol) | akchr8$hgnc_symbol=="")])
akchr8 <-  akchr8[!(is.na(akchr8$hgnc_symbol) | akchr8$hgnc_symbol==""), ] #Cleaning
...
colnames(df_raw2) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Infancy", "Infancy/Child", "Child", "Adolescence", "Adult")
#PLOT
levels(colnames(df_raw2)) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Inf", "Infancy/Child", "Child", "Adolescence", "Adult")
...
#Akey in Sestan data
```{r}
mRNAseqData=read.table("~/desertsHomo/1.data/input_data/mRNA-seq_hg38.gencode21.wholeGene.geneComposite.STAR.nochrM.gene.RPKM.normalized.CQNCombat.txt",sep="\t",header=TRUE)
...
library(readxl)
metadatamRNAseq=read_xlsx("~/desertsHomo/1.data/input_data/mRNA-seq_QC.xlsx",skip = 3)

modMetadatamRNAseq=na.omit(metadatamRNAseq)
#modMetadatamRNAseq=modMetadatamRNAseq %>% select(1:3)
modMetadatamRNAseq=modMetadatamRNAseq[,c(1,2,3)]
modMetadatamRNAseq=as.data.frame(modMetadatamRNAseq)
...
finalakeySestan1_w_sd <- finalakeySestan1 %>%
    group_by(Label) %>%
...
colnames(preli1) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Infancy", "Infancy/Child", "Child", "Adolescence", "Adult")
#PLOT
levels(colnames(preli1)) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Inf", "Infancy/Child", "Child", "Adolescence", "Adult")

#write.csv(preli1, file="~/desertsHomo/1.data/median_tables/median_Akey_Sestan.csv", row.names = FALSE)



dfakeySestan1 <- data.frame(Structure=finalakeySestan1_w_sd$Label, Means=rowMeans(finalakeySestan1_w_sd[,-1]))
...
#write.csv(preli1, file=paste0("~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/1.data/median_tables/sd_Akey_Sestan.csv"), row.names = FALSE)
...
#Filtering for chr1 alone
akchr1_table <- modsb1 %>% filter(modsb1$Genename %in% akchr1$hgnc_symbol)
#Filtering for chr3 alone
akchr3_table <- modsb1 %>% filter(modsb1$Genename %in% akchr3$hgnc_symbol)
#Filtering for chr7 alone
akchr7_table <- modsb1 %>% filter(modsb1$Genename %in% akchr7$hgnc_symbol)
#Filtering for chr8 alone
akchr8_table <- modsb1 %>% filter(modsb1$Genename %in% akchr8$hgnc_symbol)

library(readxl)
metadatamRNAseq=read_xlsx("~/desertsHomo/1.data/input_data/mRNA-seq_QC.xlsx",skip = 3)
  
data_plot <- function(akey_chr){
    
  akey_chr=t(akey_chr)
  
  #As dataframe
  akey_chr=as.data.frame(akey_chr)
  
  colnames(akey_chr) <- as.matrix(unlist(akey_chr[1,]))
  akey_chr <- akey_chr[-1, ]
  
  akey_chr <- cbind(info = rownames(akey_chr), akey_chr)
  rownames(akey_chr) <- 1:nrow(akey_chr)
  
  #duplicated columns - issue in raw data. Here: 
  colnames(akey_chr)[duplicated(colnames(akey_chr))] #1
  akey_chr <- akey_chr[, !duplicated(colnames(akey_chr))]
  
  akey_chr=akey_chr %>% 
    separate(info, c("Braincode","Regioncode"))
  
  #Transformation
  logakey_chr <- akey_chr
  
  cols.num <- colnames(logakey_chr[3:ncol(logakey_chr)])
  
  logakey_chr[,cols.num] <- lapply(logakey_chr[cols.num],as.character)
  logakey_chr[,cols.num] <- lapply(logakey_chr[cols.num],as.numeric)
  
  logakey_chr[3:ncol(logakey_chr)] <- log2(logakey_chr[3:ncol(logakey_chr)]+1)
  ...
  #With log
  finalakey_chr=merge(modMetadatamRNAseq,logakey_chr,by=c("Braincode", "Regioncode")) #input

  finalakey_chr$Braincode <- NULL
  finalakey_chr$Label<- paste(finalakey_chr$Regioncode, finalakey_chr$Window, sep="_")
  finalakey_chr$Regioncode <- NULL
  finalakey_chr$Window <- NULL
  
  finalakey_chr <- as_tibble(finalakey_chr)
  
  finalakey_chr_w_mean <- finalakey_chr %>%
      group_by(Label) %>%
      dplyr::summarise_all(median, na.rm=TRUE)
  
  #For plotting
  dfakey_chr <- data.frame(Structure=finalakey_chr_w_mean$Label, Means=rowMeans(finalakey_chr_w_mean[,-1]))
  dfakey_chr <- dfakey_chr %>% 
    separate(Structure, c("Structure","Window"))
  preli1 <- pivot_wider(dfakey_chr, names_from = Window, values_from = Means)
  preli1[,10] <- NULL
  preli1 <- preli1[complete.cases(preli1), ]
  colnames(preli1) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Infancy", "Infancy/Child", "Child", "Adolescence", "Adult")
  #PLOT
  levels(colnames(preli1)) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Inf", "Infancy/Child", "Child", "Adolescence", "Adult")
  write.csv(preli1, file=paste0("~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/1.data/median_tables/median_Akeychr",1,"_Sestan.csv"), row.names = FALSE)
  return(preli1)
}



akey_chr1_plot <- data_plot(akchr1_table)
aa <- ggparcoord(akey_chr1_plot,
           columns = 2:9, groupColumn = 1, showPoints = TRUE, scale = "globalminmax",title="Genes in Deserts chromosome 1")+theme(plot.title = element_text(size=10))+xlab("")+ylab("expression")


akey_chr3_plot <- data_plot(akchr3_table)
bb <- ggparcoord(akey_chr3_plot,
           columns = 2:9, groupColumn = 1, showPoints = TRUE, scale = "globalminmax",title="Genes in Deserts chromosome 3")+theme(plot.title = element_text(size=10))+xlab("")+ylab("expression")


akey_chr7_plot <- data_plot(akchr7_table)
cc <-ggparcoord(akey_chr7_plot,
           columns = 2:9, groupColumn = 1, showPoints = TRUE, scale = "globalminmax",title="Genes in Deserts chromosome 7")+theme(plot.title = element_text(size=10))+xlab("")+ylab("expression")


akey_chr8_plot <- data_plot(akchr8_table)
dd <- ggparcoord(akey_chr8_plot,
           columns = 2:9, groupColumn = 1, showPoints = TRUE, scale = "globalminmax",title="Genes in Deserts chromosome 8")+theme(plot.title = element_text(size=10))+xlab("")+ylab("expression")
...
finalakeypeySestan_w_mean <- finalakeypeySestan %>%
    group_by(Label) %>%
    dplyr::summarise_all(median, na.rm=TRUE)
finalakeypeySestan_w_sd <- finalakeypeySestan %>%
    group_by(Label) %>%
    dplyr::summarise_all(sd, na.rm=TRUE)
...
colnames(preli2) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Infancy", "Infancy/Child", "Child", "Adolescence", "Adult")
#PLOT
levels(colnames(preli2)) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Inf", "Infancy/Child", "Child", "Adolescence", "Adult")

#write.csv(preli2, file="~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/1.data/median_tables/median_AkeyPey_Sestan.csv", row.names = FALSE)

#For plotting_1
dfakeypeySestan <- data.frame(Structure=finalakeypeySestan_w_sd$Label, Means=rowMeans(finalakeypeySestan_w_sd[,-1]))
...
write.csv(preli2, file="~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/1.data/median_tables/sd_AkeyPey_Sestan.csv", row.names = FALSE)
...
# Desviation in the first plot // Not used for to generate the plot
# mat = matrix(ncol = 6, nrow = 0)
# 
# new_traj_fig<-data.frame(mat)
# colnames(new_traj_fig)<-c("Structure","Stage","dataf","sdf","datasetf")
# stage<-1
# for (i in colnames(traj_fig)[2:9]){
#   Structure<-unlist(as.list(traj_fig[1]))
#   Stage<-rep(i,nrow(traj_fig))
#   stage<-stage+1
#   dataf<-unlist(as.list(traj_fig[i]))
#   sdf<-unlist(as.list(traj_fig[i]))
#   datasetf<-unlist(as.list(traj_fig[10]))
#   temp<-data.frame(Structure,Stage,dataf,sdf,datasetf)
#   new_traj_fig<-rbind(new_traj_fig,temp)
#   
# }
# 
# new_traj_fig$Stage = factor(new_traj_fig$Stage, 
#     levels=c("Fetal_1","Fetal_2","Fetal_3","Birth/Infancy","Infancy/Child", "Child","Adolescence", "Adult"))
# new_traj_fig$datasetf=factor(new_traj_fig$datasetf, levels=c("Deserts", "Deserts and Pos. Sel.","Global profile"))
# 
# figtop1 <-ggplot(new_traj_fig, aes(Stage, dataf, colour=datasetf)) +
#   geom_errorbar(aes(ymax=dataf+sdf/10, ymin=dataf-sdf/10)) +
#   geom_line(aes(color=datasetf, group=datasetf),size=0.8)+
#   geom_point() +
#   facet_wrap(~Structure)+theme(plot.title = element_text(size=10), axis.text.x = element_text(angle = 45,  hjust = 1))+xlab("")+ylab("expression")+labs(color="Dataset")
...
# frame
new_traj_fig_akey<-data.frame(mat)
colnames(new_traj_fig_akey)<-c("Structure","Stage","dataf","sdf","datasetf")
stage<-1
for (i in colnames(testmean)[2:9]){
  Structure<-unlist(as.list(testmean[1]))
  Stage<-rep(i,nrow(testmean))
  stage<-stage+1
  dataf<-unlist(as.list(testmean[i]))
  sdf<-unlist(as.list(testsd[i]))
  datasetf<-unlist(as.list(testmean[10]))
  temp<-data.frame(Structure,Stage,dataf,sdf,datasetf)
  new_traj_fig_akey<-rbind(new_traj_fig_akey,temp)
  
}

new_traj_fig_akey$Stage = factor(new_traj_fig_akey$Stage, 
    levels=c("Fetal_1","Fetal_2","Fetal_3","Birth/Infancy","Infancy/Child", "Child","Adolescence", "Adult"))
new_traj_fig_akey$datasetf=factor(new_traj_fig_akey$datasetf, levels=c("Deserts", "Deserts and Pos. Sel.","Global profile"))

figtop2<-ggplot(new_traj_fig_akey, aes(Stage, dataf, colour=Structure)) +
  geom_line(aes(color=Structure, group=Structure),size=0.8)+
  geom_point() +
  theme(plot.title = element_text(size=10),legend.position = "none", axis.text.x = element_text(angle = 45,  hjust = 1))+ xlab("")+ ylab("expression")+ 
  ggtitle("B")
...
# converting the matrix to data
# frame
new_traj_fig_akeypey<-data.frame(mat)
colnames(new_traj_fig_akeypey)<-c("Structure","Stage","dataf","sdf","datasetf")
stage<-1
for (i in colnames(testmean)[2:9]){
  Structure<-unlist(as.list(testmean[1]))
  Stage<-rep(i,nrow(testmean))
  ...
  datasetf<-unlist(as.list(testmean[10]))
  temp<-data.frame(Structure,Stage,dataf,sdf,datasetf)
  new_traj_fig_akeypey<-rbind(new_traj_fig_akeypey,temp)
  
}

new_traj_fig_akeypey$Stage = factor(new_traj_fig_akeypey$Stage, 
    levels=c("Fetal_1","Fetal_2","Fetal_3","Birth/Infancy","Infancy/Child", "Child","Adolescence", "Adult"))
new_traj_fig_akeypey$datasetf=factor(new_traj_fig_akeypey$datasetf, levels=c("Deserts", "Deserts and Pos. Sel.","Global profile"))

figtop3<-ggplot(new_traj_fig_akeypey, aes(Stage, dataf, colour=Structure)) +
  geom_line(aes(color=Structure, group=Structure),size=0.8)+
  geom_point() +
  theme(plot.title = element_text(size=10), axis.text.x = element_text(angle = 45,  hjust = 1))+ xlab("")+ ylab("expression")+ 
  ggtitle("C")
...
ggsave(file="~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/2.plots/Sestan_AkeyPey_log2_median/fig_combined_dev.pdf", fig2_main, width = 11.69, height = 8.27, units = "in")
```

#Figure 2.1 ans 2.2 . Details

```{r}

#Akey
# Graphic with geomerrorbar
# ggplot(new_traj_fig_akey, aes(x=Stage, y=dataf, colour=Structure)) +
#   geom_errorbar(aes(ymax=dataf+sdf, ymin=dataf-sdf)) + 
#   geom_line(aes(color=Structure, group=Structure),size=0.8)+
#   facet_wrap(~Structure)+
#   geom_point()+
#   theme(plot.title = element_text(size=10), axis.text.x = element_text(angle = 45,  hjust = 1))+xlab("")+ylab("expression")+labs(color="Dataset")
...
figtop2<-ggplot(new_traj_fig_akey, aes(Stage, dataf, colour=Structure)) +
  ...
  theme(plot.title = element_text(size=10),legend.position = "none", axis.text.x = element_text(angle = 45,  hjust = 1))+ xlab("")+ ylab("expression")+ 
  ggtitle("A")
sdfigop2 <-ggplot(new_traj_fig_akey, aes(x=Stage, y=dataf, colour=Structure,fill=Structure)) +
  #geom_errorbar(aes(ymax=dataf+sdf, ymin=dataf-sdf)) + 
  geom_line(aes(color=Structure, group=Structure),size=0.8)+
  facet_wrap(~Structure,nrow=3,ncol=2)+
  geom_ribbon(data=new_traj_fig_akey, aes(ymax=dataf+sdf, ymin=dataf-sdf, group = Structure), alpha = 0.2)+
  geom_point()+
  theme(plot.title = element_text(size=10), axis.text.x = element_text(angle = 45,  hjust = 1))+xlab("")+ylab("expression")+ 
  ggtitle("B")
...
grid.arrange(figtop2, sdfigop2, layout_matrix = lay)
fig2_main <- grid.arrange(figtop2, sdfigop2, layout_matrix = lay)
ggsave(file="~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/2.plots/Sestan_AkeyPey_log2_median/fig_combined_dev_akey.pdf", fig2_main, width = 11.69, height = 8.27, units = "in")
#Akey
# Graphic with geomerrorbar
# ggplot(new_traj_fig_akeypey, aes(x=Stage, y=dataf, colour=Structure)) +
#   geom_errorbar(aes(ymax=dataf+sdf, ymin=dataf-sdf)) + 
#   geom_line(aes(color=Structure, group=Structure),size=0.8)+
#   facet_wrap(~Structure)+
#   geom_point()+
...
#Using geomribbon
figtop2<-ggplot(new_traj_fig_akeypey, aes(Stage, dataf, colour=Structure)) +
  geom_line(aes(color=Structure, group=Structure),size=0.8)+
  geom_point() +
  theme(plot.title = element_text(size=10),legend.position = "none", axis.text.x = element_text(angle = 45,  hjust = 1))+ xlab("")+ ylab("expression")+ 
  ggtitle("A")
sdfigop2 <-ggplot(new_traj_fig_akeypey, aes(x=Stage, y=dataf, colour=Structure,fill=Structure)) +
  #geom_errorbar(aes(ymax=dataf+sdf, ymin=dataf-sdf)) + 
  geom_line(aes(color=Structure, group=Structure),size=0.8)+
  facet_wrap(~Structure,nrow=3,ncol=2)+
  geom_ribbon(data=new_traj_fig_akeypey, aes(ymax=dataf+sdf, ymin=dataf-sdf, group = Structure), alpha = 0.2)+
  geom_point()+
  theme(plot.title = element_text(size=10), axis.text.x = element_text(angle = 45,  hjust = 1))+xlab("")+ylab("expression")+ 
  ggtitle("B")
...
grid.arrange(figtop2, sdfigop2, layout_matrix = lay)
fig2_main <- grid.arrange(figtop2, sdfigop2, layout_matrix = lay)
ggsave(file="~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/2.plots/Sestan_AkeyPey_log2_median/fig_combined_dev_akeypey.pdf", fig2_main, width = 11.69, height = 8.27, units = "in")
...
preli1 <- read.csv(file="~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/1.data/median_tables/median_Akey_Sestan.csv")
preli1 <- as_tibble(preli1) #AKEY

preli1sd <- read.csv(file="~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/1.data/median_tables/sd_Akey_Sestan.csv")
preli1sd <- as_tibble(preli1sd) #AKEY

preli2 <- read.csv(file="~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/1.data/median_tables/median_AkeyPey_Sestan.csv")
preli2 <- as_tibble(preli2) #AKEYpey

preli2sd <- read.csv(file="~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/1.data/median_tables/sd_AkeyPey_Sestan.csv")
preli2sd <- as_tibble(preli2sd) #AKEYpey

preli3 <- read.csv(file="~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/1.data/median_tables/median_filtered_rawSestan.csv")
preli3 <- as_tibble(preli3) #rawfiltered

colnames(preli1) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Infancy", "Infancy/Child", "Child", "Adolescence", "Adult")
#PLOT
levels(colnames(preli1)) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Infancy", "Infancy/Child", "Child", "Adolescence", "Adult")

colnames(preli1sd) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Infancy", "Infancy/Child", "Child", "Adolescence", "Adult")
#PLOT
levels(colnames(preli1sd)) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Infancy", "Infancy/Child", "Child", "Adolescence", "Adult")


colnames(preli2) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Infancy", "Infancy/Child", "Child", "Adolescence", "Adult")
#PLOT
levels(colnames(preli2)) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Infancy", "Infancy/Child", "Child", "Adolescence", "Adult")


colnames(preli2sd) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Infancy", "Infancy/Child", "Child", "Adolescence", "Adult")
#PLOT
levels(colnames(preli2sd)) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Infancy", "Infancy/Child", "Child", "Adolescence", "Adult")

colnames(preli3) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Infancy", "Infancy/Child", "Child", "Adolescence", "Adult")
#PLOT
levels(colnames(preli3)) <- c("Structure", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Infancy", "Infancy/Child", "Child", "Adolescence", "Adult")

preli1$dataset <- c("Deserts")
preli1mean<-preli1
preli1sd$dataset <- c("Deserts")
preli2$dataset <- c("Deserts and Pos. Sel.")
preli2mean<-preli2
preli2sd$dataset <- c("Deserts and Pos. Sel.")
...
colnames(prelites2) <- c("Structure", "Genename", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Infancy", "Infancy/Child", "Child", "Adolescence", "Adult")
#PLOT
levels(colnames(prelites2)) <- c("Structure", "Genename", "Fetal_1", "Fetal_2", "Fetal_3", "Birth/Inf", "Infancy/Child", "Child", "Adolescence", "Adult")
