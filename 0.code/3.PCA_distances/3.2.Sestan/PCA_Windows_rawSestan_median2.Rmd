...
#Ensembl - obtaining lists of genes
...
#255
length(results$hgnc_symbol[!(is.na(results$hgnc_symbol) | results$hgnc_symbol=="")] %in% results$hgnc_symbol)
results$hgnc_symbol[!(is.na(results$hgnc_symbol) | results$hgnc_symbol=="")] %in% results$hgnc_symbol

length(results)
results_akeychr<-list()
results_akeychr[[1]]<-results[results$chromosome_name==1,]
results_akeychr[[3]]<-results[results$chromosome_name==3,]
results_akeychr[[7]]<-results[results$chromosome_name==7,]
results_akeychr[[8]]<-results[results$chromosome_name==8,]
...
both_chr<-list()
both_chr[[1]]<-both[both$chromosome_name==1,]
both_chr[[3]]<-both[both$chromosome_name==3,]
both_chr[[7]]<-both[both$chromosome_name==7,]
both_chr[[8]]<-both[both$chromosome_name==8,]
...
# Setting data varaiables depending on the selection of genes
...
# results - (akey)/ both - (akey+pey) / resultsPeynotAkey / (withoufilter whole)
...
finalselectionDataAkey

```

# Setting data varaiables depending on the selection of genes by chromosomes
...
#Filtering for Akey alone
# results_akeychr - (akey)/ 
finalselectionDataAkeyChr<-list()


#Filtering for Akey and Pey
# both_chr - (akey)/ 
#finalselectionDataAkeyPeyChr<-list()

for (chr in c(1,3,7,8)) {
selectionData <- modsb1  %>% filter(modsb1$Genename %in% results_akeychr[[chr]]$hgnc_symbol)
selectionData=t(selectionData)
...
#With log
finalselectionDataAkeyChr[[chr]]=merge(modMetadatamRNAseq,logselectionData,by=c("Braincode", "Regioncode"))
finalselectionDataAkeyChr[[chr]]$Braincode <- NULL #important
#Without log:
#finalselectionData=merge(modMetadatamRNAseq,akeypeySestan,by=c("Braincode", "Regioncode"))
...
akeySestan <- finalselectionDataAkeyChr[[8]]
...
write.csv(pvalues, file="C:\\Users\\Usuario\\Documents\\Master_CCiL_2\\Interdisc\\tesina\\tesina_data_code\\github_code\\desertsHomo\\1.data\\distances_pvalues\\PCA_sig_components\\JS_pvaluesAkeyChr8.csv")
...
ggsave(supplfig_x, file="~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/2.plots/Sestan_PCA_distances_JS/JackStraw_Sestan_AkeyChr8_supplfig.pdf", width = 11.69, height = 8.27, units = "in")
jackStrawPCAAkey=c(0,10,9,9,9,9,8,7,9)
jackStrawPCAAkeyChr1=c(0,7,6,7,5,6,6,6,8)
jackStrawPCAAkeyChr3=c(0,1,1,1,1,1,1,1,1)#not enough values
jackStrawPCAAkeyChr7=c(0,6,3,4,4,4,4,2,4)
jackStrawPCAAkeyChr8=c(0,2,3,3,3,3,3,3,5)
...
```
#JackStraw for by chromosomes in Akey
```{r}


correspStage<-list()
correspStage[[1]]<-"chr_1"
correspStage[[3]]<-"chr_3"
correspStage[[7]]<-"chr_7"
correspStage[[8]]<-"chr_8"


jackSplot<-list()
for( i in c(1,3,7,8)){
  
  windowPCA<- finalselectionDataAkeyChr[[i]]
...
# ~/AA_investigation/Introgression_deserts/code/new_july/JSPCA_Sestan_distances/
write.csv(pvalues, file="C:\\Users\\Usuario\\Documents\\Master_CCiL_2\\Interdisc\\tesina\\tesina_data_code\\github_code\\desertsHomo\\1.data\\distances_pvalues\\PCA_sig_components\\JS_pvaluesAkeyChr.csv")

write.table(pvalues,"C:\\Users\\Usuario\\Documents\\prueba.csv")
file.access("C:\\Users\\Usuario\\Documents\\AA_investigation\\Introgression_deserts\\code\\new_july\\JS_PCA_Sestan_distances", 2)
setwd("C:\\Users\\Usuario\\Documents\\AA_investigation\\Introgression_deserts\\code\\new_july\\JS_PCA_Sestan_distances")
getwd()
supplfig_x<-ggarrange(jackSplot[[1]],jackSplot[[3]],
                      jackSplot[[7]],jackSplot[[8]])
ggsave(supplfig_x, file="~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/2.plots/Sestan_PCA_distances_JS/JackStraw_Sestan_AkeyChr_supplfig.pdf", width = 11.69, height = 8.27, units = "in")

jackStrawByChr=c(11,0,2,0,0,0,8,7)
# 15 PCA in the four chr appear as reliable
```
...
#finalselectionDataWhole<-finalselectionData
akeySestan <-finalselectionDataAkeyChr[[8]] #Input from jm_Allen
jackStraw<-jackStrawPCAAkeyChr8 # jackStrawPCAAkey
...
write.csv(willCoxPvals, file="~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/1.data/distances_pvalues/wilcox_akeyChr8Sestan.csv")
...
supplfig_x <- ggarrange(boxplotsDist[[2]], boxplotsDist[[3]],boxplotsDist[[4]],
          ...
          common.legend = TRUE, legend = "right")
#supplfig_x
ggsave(supplfig_x, file="~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/2.plots/Sestan_raw_filtered_median2_trajectories/filtered_AkeySestanChr8_supplfig.pdf", width = 11.69, height = 8.27, units = "in")
...
# Data analysis by chromosomes Akey
```{r}
setwd("~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code")
...
#Pairwise wilcox tests
wilcoxTests<-data.frame(matrix(ncol=24,nrow = 6))
wilcoxTestsCol<-c()
for (wind in c(1,3,7,8)){
  for (str in keys(structDist)){
    wilcoxTestsCol<-append(wilcoxTestsCol,paste(str,"_",wind))
  }
}
...
willCoxPvals$window <- str_replace(willCoxPvals$window, "1", "chr_1")
willCoxPvals$window <- str_replace(willCoxPvals$window, "3", "chr_3")
willCoxPvals$window <- str_replace(willCoxPvals$window, "7", "chr_7")
willCoxPvals$window <- str_replace(willCoxPvals$window, "8", "chr_8")
willCoxPvals
willCoxPvals$window = factor(willCoxPvals$window, 
    levels=c("chr_1","chr_3","chr_7","chr_8"))
...
write.csv(willCoxPvals, file="~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/1.data/distances_pvalues/wilcox_akeyChrSestan.csv")
...
supplfig_x <- ggarrange(boxplotsDist[[1]], boxplotsDist[[3]],boxplotsDist[[7]],
          boxplotsDist[[8]], subfigx,common.legend = TRUE, legend = "right")
supplfig_x
ggsave(supplfig_x, file="~/Master_CCiL_2/Interdisc/tesina/tesina_data_code/github_code/desertsHomo/2.plots/Sestan_raw_filtered_median2_trajectories/filtered_AkeySestanChr_supplfig.pdf", width = 11.69, height = 8.27, units = "in")
...
wilcox_AkeyPeySestan$Structure <- paste0(wilcox_AkeyPeySestan$Structure," - Chen et al. 2020 & Peyregne et al. 2017")
...
stage<- "Fetal_1" # Adult, Adolescence, Fetal_1
...
willcoxStage$Datasets<-c("Chen et al. 2020", "Chen et al. 2020 & Peyregne et al. 2017","Peyregne et al. 2017 not in Chen et al. 2020","Li et al. 2018 (Whole)")
willcoxStage$log2pvalAVG<- (-1)*willcoxStage$log2pvalAVG
histogramStr2<-ggplot(willcoxStage, aes(Datasets, log2pvalAVG, fill = Datasets)) + theme(legend.position="bottom",legend.direction = "vertical",axis.text.y=element_blank(),
  ...
  scale_fill_manual(values =   c("Chen et al. 2020" = "#7CD0FE",
                               ...
                               "Peyregne et al. 2017 not in Chen et al. 2020" = "#7CFEAD",
                               "Li et al. 2018 (Whole)" = "#FEA05E"))+ggtitle("C)", subtitle=stage)+coord_flip()+ theme(text = element_text(size=17))
histogramCereb
# histogramMdt
...
graphSupp[[i]] <- ggplot(wilcoxStruct, aes(x=window, y=log2pvalAVG, group=Structure)) +
  geom_line(aes(color=Structure))+
  geom_point(aes(color=Structure))+xlab("")+ylab("log2pvalAVG")+theme(plot.title = element_text(size=17), text = element_text(size=17), axis.text.x = element_text(angle = 45,  hjust = 1), legend.position = "right")+ggtitle("C)")+geom_hline(yintercept = log2(0.01), colour="black", size=0.5, alpha=0.5) +
  scale_color_manual(labels = c("Chen et al. 2020", "Chen et al. 2020 & Peyregne et al. 2017","Peyregne et al. 2017 not in Chen et al. 2020","Li et al. 2018 (Whole)"), values = c("blue", "red","green","brown"))+labs(color='Dataset')+ theme(text = element_text(size=17))
}
...
PCAPlot<-ggplot(PCi[,1:2], aes(x=PC1, y=PC2, color=PCi$topStructure)) + 
    geom_point(size=2) + ggtitle("A)")+labs(color='Structure') + theme(legend.position = "none", text = element_text(size=17))#
PCAPlot
#PCAPlotAkeyPey<-PCAPlotMdt
#PCAPlotAkey<-PCAPlot
# wilcox_AkeySestan
wilcox_AkeySestan$window <- factor(wilcox_AkeySestan$window, 
    levels=c("Fetal_1","Fetal_2","Fetal_3","Birth/Inf","Inf/Child", "Child","Adolescence", "Adult"))
wilcox_AkeyPeySestanPC2$window <- factor(wilcox_AkeyPeySestanPC2$window, 
    levels=c("Fetal_1","Fetal_2","Fetal_3","Birth/Inf","Inf/Child", "Child","Adolescence", "Adult"))
# subfigxAkey
# wilcox_AkeyPeySestan or wilcox_AkeySestan
subfigx <- ggplot(wilcox_AkeyPeySestanPC2, aes(x=window, y=log2pvalAVG, group=Structure)) +
  geom_line(aes(color=Structure))+
  geom_point(aes(color=Structure))+xlab("")+ylab("log2pvalAVG")+theme( axis.text.x = element_text( angle = 45,  hjust = 1), text = element_text(size=17), legend.position = "bottom", legend.direction = "vertical", legend.title=element_text(size=15), legend.text=element_text(size=11))+ggtitle("B)")+geom_hline(yintercept = log2(0.01), colour="black", size=0.5, alpha=0.5)
subfigxAkeyPey2PC
grid.arrange(figtop1, figtop2, figtop3, layout_matrix = lay)
# histogram <--> graphSupp[[1]]
fig2_main <- grid.arrange(PCAPlot, subfigx, histogramStr2, 
                          layout_matrix = lay)

ggsave(file="fig_combined2_PCA_Adolescent_Struct_MDT_AkeyPey_size.pdf", fig2_main, width = 20, height = 8.27, units = "in")
getwd()
