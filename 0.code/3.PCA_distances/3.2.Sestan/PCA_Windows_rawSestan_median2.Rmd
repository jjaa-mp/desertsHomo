...
akeySestan <- finalselectionDataPeyAkey
...
ggsave(supplfig_x, file="~/AA_investigation/Introgression_deserts/code/new_july/JS_PCA_Sestan_distances/JackStraw_Sestan_PeyAkey_supplfig.pdf", width = 11.69, height = 8.27, units = "in")
...
#akeySestan <- from jm_Allen finalraw2 inputPC
  ...
  #numPCA<-jackStrawPCAAkey[[i]] #jackstrawNumber of selected elements
  numPCA<-15
...
graphSupp[[i]] <- ggplot(wilcoxStruct, aes(x=window, y=log2pvalAVG, group=Structure)) +
  ...
  scale_color_manual(labels = c("Akey", "AkeyPey","PeyNotAkey","Whole"), values = c("blue", "red","green","brown"))
}
...
wilcox_AkeyPeySestan$window <- factor(wilcox_AkeyPeySestan$window, 
    levels=c("Fetal_1","Fetal_2","Fetal_3","Birth/Inf","Inf/Child", "Child","Adolescence", "Adult"))
subfigx <- ggplot(wilcox_AkeyPeySestan, aes(x=window, y=log2pvalAVG, group=Structure)) +
  geom_line(aes(color=Structure))+
...
fig2_main <- grid.arrange(PCAPlot, graphSupp[[6]], subfigx, layout_matrix = lay)
ggsave(file="../../../2.plots/fig_combined_PCA_Struct_Thalamus_AkeyPey.pdf", fig2_main, width = 11.69, height = 8.27, units = "in")
```