...
willcoxStage<-wilcoxStruct%>%
  filter(str_detect(window, "Adult"))
...
histogram<-ggplot(willcoxStage, aes(Datasets, log2pvalAVG, fill = Datasets)) +     # Manually specifying colors
  geom_bar(stat = "identity") +xlab("")+ ylab("-log2pvalAVG")+ 
  scale_fill_manual(values =   c("Akey" = "#7CD0FE",
                               "AkeyPey" = "#FE7C7C",
                               "PeyNotAkey" = "#7CFEAD",
                               "whole" = "#FEA05E"))+ggtitle("B)")
histogram
...
graphSupp[[i]] <- ggplot(wilcoxStruct, aes(x=window, y=log2pvalAVG, group=Structure)) +
  geom_line(aes(color=Structure))+
  geom_point(aes(color=Structure))+xlab("")+ylab("log2pvalAVG")+theme(plot.title = element_text(size=10), axis.text.x = element_text(angle = 45,  hjust = 1), legend.position = "right")+ggtitle("B)")+geom_hline(yintercept = log2(0.01), colour="black", size=0.5, alpha=0.5) +
  scale_color_manual(labels = c("Akey", "AkeyPey","PeyNotAkey","Whole"), values = c("blue", "red","green","brown"))+labs(color='Dataset')
}
...
fig2_main <- grid.arrange(PCAPlot, histogram, subfigx, layout_matrix = lay)
fig2_main
PCAPlot
ggsave(file="../../../2.plots/new_vers_fig_combined2_PCA_Adult_Struct_Cerebellum_Akey.pdf", fig2_main, width = 20, height = 8.27, units = "in")
```