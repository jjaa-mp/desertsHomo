...
#finalselectionDataWhole<-finalselectionData
akeySestan <-finalselectionDataAkey #Input from jm_Allen
jackStraw<-jackStrawPCAAkey # jackStrawPCAAkey
...
struct<-4 # Cerebellum =1, striatum=4, MDT=6
stage<- "Adolescence" # Adult, Fetal_1, Adolescence
...
graphSupp[[i]] <- ggplot(wilcoxStruct, aes(x=window, y=log2pvalAVG, group=Structure)) +
  ...
  scale_color_manual(labels = c("Chen et al. 2020", "Chen et al. 2020 & Peyregne et al. 2017","Peyregne et al. 2017 not in Chen et al. 2020","Li et al. 2018 (Whole)"), values = c("blue", "red","green","brown"))+labs(color='Dataset')
}
...
wilcox_AkeySestan$window <- factor(wilcox_AkeySestan$window, 
    levels=c("Fetal_1","Fetal_2","Fetal_3","Birth/Inf","Inf/Child", "Child","Adolescence", "Adult"))
# subfigxAkey
subfigx <- ggplot(wilcox_AkeySestan, aes(x=window, y=log2pvalAVG, group=Structure)) +
  geom_line(aes(color=Structure))+
...
ggsave(file="../../../2.plots/combined_figures/fig_combined2_PCA_Adolescent_Struct_Striatum_Akey.pdf", fig2_main, width = 20, height = 8.27, units = "in")
```