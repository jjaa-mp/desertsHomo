...
par(mfrow=c(3,2))
plot2 <- plotFeature(finalcbc,tVectIn = time.vector,featureNames = c("SYT6", "ROBO2", "CADPS2"),trendyOutData = res, customLabels.x = c("Fetal_1","Fetal_3","Child","Adult" ), xlab="")
dev.off()
```
...
#Selected genes
par(mfrow=c(2,2))
plot2 <- plotFeature(finalstr,tVectIn = time.vector, featureNames = c("ROBO2", "SYT6", "ST7"),trendyOutData = res)

pdf("~/desertsHomo/2.plots/trendy_segmentedRegression/STR_GenesDesertPosSel.pdf")
par(mfrow=c(3,2))
plot2 <- plotFeature(finalstr,tVectIn = time.vector,featureNames = c("ROBO2", "ST7", "SYT6"),trendyOutData = res, customLabels.x = c("Fetal_1","Fetal_3","Child"), xlab="")
dev.off()
```
